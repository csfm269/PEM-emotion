...
#################### diagnosis ###############
# joint training 10 fold CV
ind.auc = cbind( sapply(rf.res$ind, function(x) auc(x$data$y, x$data$p.stack)),
       sapply(rf.res$ind, function(x) auc(x$data$y, x$data$V1)), 
       sapply(rf.res$ind, function(x) sum(x$data$y==1)),
       sapply(rf.res$ind, function(x) length(x$data$y)) )
ind.auc
roc(all.pred$y, all.pred$p.stack)
roc(all.pred$y, all.pred$p.in)
specialist.auc.all[[3]]$data
plot(ind.auc[,3]/ind.auc[,4], ind.auc[,3], col = (ind.auc[,1] - ind.auc[,2]))
plot(ind.auc[,3]/ind.auc[,4], ind.auc[,1] - ind.auc[,2])
cbind(ind.auc[,3]/ind.auc[,4], ind.auc[,1] - ind.auc[,2])
abline(h = 0)

lm(ind.auc[,1] - ind.auc[,2]~ind.auc[,3] + ind.auc[,4])
...
logistic.loss = function(y, p.pred){
  ...
  n.pos = sum(y==1)
  # if(n.pos<= 5){
  #   -mean(y*log(p.pred)*20 + (1-y)*log(1-p.pred))
  # }else{
  #   -mean(y*log(p.pred) + (1-y)*log(1-p.pred))
  # }
  -mean(y*log(p.pred) + (1-y)*log(1-p.pred))
...
rf.auc = sapply(1:15, function(idx){
  print(idx)
  X = model.matrix(formula, data = data.ls[[idx]])[,-1]
  y = as.factor(data.ls[[idx]][[outcome]])
  
  folds = createFolds(y, k = 10)
  tmp.4 = lapply(folds, function(fold){
    md = randomForest(x = X[-fold,,drop = F], y = y[-fold])
    w = specialist.wrapper(X[-fold,,drop=F], y[-fold], learner.lst, all.models[-idx], gamma = 0.4)
    pred.all = sapply( predict(c(list(md), all.models[-idx]), X[fold,,drop = F], type = "prob"), function(y){
      y[,2]
    })
    p.in = predict(md, X[fold,,drop = F], type = "prob")[,2]
    list(w = w, pred.all = pred.all, pred.stack = pred.all%*%w, p.in = p.in)
  })
  
  p.stack = unlist(sapply(tmp.4, function(x) x$pred.stack))
  p.in = unlist(sapply(tmp.4, function(x) x$p.in))
  plot(p.stack, p.in)
  
  c(auc(y[unlist(folds)], unlist(sapply(tmp.4, function(x) x$pred.stack))),
  auc(y[unlist(folds)], unlist(sapply(tmp.4, function(x) x$p.in))))
})

rep.2 = sapply(1:10, function(rep){
  idx = 13
  print(rep)
  X = model.matrix(formula, data = data.ls[[idx]])[,-1]
  y = as.factor(data.ls[[idx]][[outcome]])
...
plot(unlist(sapply(tmp.4, function(x) x$pred.stack)), unlist(sapply(tmp.4, function(x) x$pred.all[,1])), col = y)
abline(a = 0, b = 1)
plot(roc1)
plot(roc2)

# w = nnls(A = t(tmp.4), b = data.ls[[4]]$ang.extreme)$x
w.out = w.simplex(t(sapply(tmp.4, function(x) x$pred.all)), as.numeric(y)-1, L = 1, gamma = 0.2)
auc( y, t(sapply(tmp.4, function(x) x$pred.all))%*%w.out )

ggplot(melt(cor(t(tmp.4))), aes(x = Var1, y = Var2, fill = value)) + geom_tile() + scale_fill_gradient2(low = "blue", high = "red")
cbind(data.ls[[4]]$ang.extreme, tmp.4)

cbind( sapply( specialist.auc.all, function(x) auc(x$data$y, x$data$p.stack)), 
       sapply( specialist.auc.all, function(x) auc(x$data$y, x$data$p.in)), 
       sapply( specialist.auc.all, function(x) sum(x$data$y==1)),
       sapply( specialist.auc.all, function(x) length(x$data$y)))
pr.curve(scores.class1 = all.pred$p.stack[all.pred$y==1], scores.class0 = all.pred$p.stack[all.pred$y==0], curve = T)
pr.curve(scores.class1 = all.pred$p.in[all.pred$y==1], scores.class0 = all.pred$p.in[all.pred$y==0], curve = T)

#unbiased AUC estimators
idx = 13
data.use = data.ls[[idx]]
idx.pos = (1:nrow(data.use))[data.use$ang.extreme==1]
idx.neg = (1:nrow(data.use))[data.use$ang.extreme==0]

X = model.matrix(formula, data = data.use)[,-1]
y = as.factor(data.use[[outcome]])
all.comb = expand.grid(idx.pos, idx.neg)

lambda = 0
auc.res = apply(all.comb, 1, function(x){
  print(x)
  idx.remove = c(x[1],x[2])
  md = randomForest(x = X[-idx.remove,,drop = F], y = y[-idx.remove])
  w = rowMeans(sapply(1:5, function(rep){
    specialist.wrapper(X[-idx.remove,,drop=F], y[-idx.remove], learner.lst, all.models[-idx], lambda, nfolds = 5)
  }))
  
  pred.all = sapply( predict(c(list(md), all.models[-idx]), X[idx.remove,,drop = F], type = "prob"), function(y){
    y[,2]
  })
  p.in = c(predict(md, X[idx.remove,,drop = F], type = "prob")[,2])
  p.stack = c(pred.all%*%w)
  c(p.stack[1]>p.stack[2], p.in[1]>p.in[2])
})
rowMeans(auc.res)

folds = createFolds(y, k = 10)
auc.res.loo = sapply(1:nrow(data.use), function(x){
  md = randomForest(x = X[-x,,drop = F], y = y[-x])
  predict(md, X[x,,drop = F], type = "prob")[,2]
})
auc(y, auc.res.loo)
loo.pair = apply(all.comb, 1, function(x){
  c(auc.res.loo[x[1]], auc.res.loo[x[2]])
})
mean(loo.pair[1,]>loo.pair[2,])

all.p = lapply(data.ls, function(x){
  tmp = predict(all.models, x, type = "prob")
  sapply(tmp, function(x) x[,2])
  })
all.y = unlist(lapply(data.ls, function(x) x[[outcome]]))
w.gen = w.simplex(all.p, all.y, L = 1)

fn.min = function(x){
  sum(sapply(seq_along(data.ls), function(idx){
    p.pred = all.p[[idx]]%*%x
    y = data.ls[[idx]][[outcome]]
    logistic.loss(y, p.pred)
  }))
}
w.gen.new = auglag(rep(1/length(data.ls), length(data.ls)), fn.min, hin = function(x) x, heq = function(x) sum(x)-1)$par

data.tmp = test$AH5
plot( data.tmp$day ,data.tmp$Angry)

auc.res.cv = unlist(sapply(folds, function(fold){
  md = randomForest(x = X[-fold,,drop = F], y = y[-fold])
  predict(md, X[fold,,drop = F], type = "prob")[,2]
}))

auc(y[unlist(folds)], auc.res.cv)

tmp1 = randomForest(x = X, y = y)
